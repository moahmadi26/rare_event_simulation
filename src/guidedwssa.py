...
def guidedwssa_train(model_path, N, t_max, target_index, target_value, F=None, negative_method='C'):
    ...
    for i in range(N):
        ...
        p_array.append(mn / (i + 1))
        trajectories.append((curr_traj, w, min_dist, min_dist_state))
    
    p = p_array[-1] if p_array else 0
    var = square_sum / N - p**2 if N > 0 else 0
    SE = (1 / math.sqrt(N)) * math.sqrt(var) if N > 0 and var >= 0 else 0
    zstar = 1.96
    conf = [p - zstar * SE, p + zstar * SE]
    
    return trajectories, p, var, conf
